...
describe('Exercise Performance Tests', () => {
  ...
  it('should handle case-insensitive answer checking efficiently', () => {
    ...
    expect(avgDuration).toBeLessThan(0.01); // Should be extremely fast
  });
  ...
  it('should calculate percentages efficiently', () => {
    ...
    expect(duration).toBeLessThan(150); // Adjusted for realistic performance
  });
  ...
  it('should measure memory impact of exercise state', () => {
    const mockWords = Array.from({ length: 50 }, (_, i) => ({
      word: `word${i}`,
      definition: `Definition ${i}`.repeat(10), // Larger definitions
      example: `Example ${i}`.repeat(10), // Larger examples
      timestamp: new Date().toISOString()
    }));
